...
void initZ2SDeviceRGBWLightSource(ZigbeeGateway *gateway, zb_device_params_t *device, uint8_t Supla_channel) {
...
void addZ2SDeviceRGBWLightSource(ZigbeeGateway *gateway, zb_device_params_t *device, uint8_t free_slot, char *name, uint32_t func) {
...
void msgZ2SDeviceRGBWLightSource(uint8_t Supla_channel, uint8_t id, uint8_t value, bool state) {
  ...
  if (element != nullptr && element->getChannel()->getChannelType() == SUPLA_CHANNELTYPE_DIMMERANDRGBLED) {
    ...
    //Supla_Z2S_RGBWLightSource->setRGBW(-1 /*red*/, -1 /*green*/, -1 /*blue*/, -1/*color brightness*/, map_value);     
  }
}
