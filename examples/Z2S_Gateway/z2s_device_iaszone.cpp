...
void initZ2SDeviceIASzone(
  ...
  Supla::Sensor::Z2S_VirtualBinary *Supla_Z2S_VirtualBinary = nullptr;

  if (element) {

    Supla_Z2S_VirtualBinary = static_cast<
      Supla::Sensor::Z2S_VirtualBinary *>(element);
  }
  else {

    Supla_Z2S_VirtualBinary = new Supla::Sensor::Z2S_VirtualBinary(true);
...
void addZ2SDeviceIASzone(
  ...
  Z2S_setChannelData(
    Supla_Z2S_VirtualBinary->getZ2SCorePtr(), device, free_slot,
    Supla_Z2S_VirtualBinary->getChannelNumber(), 
    SUPLA_CHANNELTYPE_BINARYSENSOR, sub_id, name, func);

  initZ2SDeviceIASzone(
    free_slot, Supla_Z2S_VirtualBinary->getZ2SChannel(), 
    Supla_Z2S_VirtualBinary->getZ2SElementPtr());

  Supla_Z2S_VirtualBinary->getChannel()->setSubDeviceId(
