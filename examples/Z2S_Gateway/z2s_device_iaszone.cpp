...
void initZ2SDeviceIASzone(int16_t channel_number_slot) {
  ...
  //if (z2s_channels_table[channel_number_slot].user_data_flags & USER_DATA_FLAG_SED_TIMEOUT == USER_DATA_FLAG_SED_TIMEOUT)
  //  timeout = z2s_channels_table[channel_number_slot].user_data_1;
...
void msgZ2SDeviceIASzone(int16_t channel_number_slot, bool state, signed char rssi, bool check_flags) {
  ...
  if (element != nullptr && element->getChannel()->getChannelType() == SUPLA_CHANNELTYPE_BINARYSENSOR) {
        ...
        if ((sendIASNotifications) && ~(z2s_channels_table[channel_number_slot].user_data_flags & USER_DATA_FLAG_DISABLE_NOTIFICATIONS))
          Supla::Notification::SendF(z2s_channels_table[channel_number_slot].Supla_channel, z2s_channels_table[channel_number_slot].Supla_channel_name,
